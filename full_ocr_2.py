...
def find_each_line(img):
    ...
    for i,contour in enumerate(contours):
        ...
        print(i,perimeter)
        if(perimeter<400 or perimeter>2500):   #have to automate this by removing outliers 
            pass
        else:
...
for i in range(len(image_list)-1,-1,-1):
    ...
    #cv2.imshow("before thining",image_list[i])
    #cv2.waitKey(0)
    #image_list[i]=thining(image_list[i])
    #cv2.imshow("after thining",image_list[i])
    #cv2.waitKey(0)
